...
const BAND_PASS_FREQUENCY = 1200;// 1800;d2
const BAND_PASS_Q = 0.5;// 1;
let audioQueue = [];
let isPlaying = false;
...
function startWebSocket(clientid) {
    websocket = new WebSocket('wsmultipal/' + clientid);
    ...
    audioContext.audioWorklet.addModule('/lib/voicecall/audio-processor.js').then(() => {
            ...
            .then(function (mediaStream) {
                ...
                audioWorkletNode.port.onmessage = function (event) {
                    ...
                    for (let i = 0; i < downsampledBuffer.length; i++) {
                        const adjustedSample = Math.max(-1, Math.min(1, downsampledBuffer[i] * 1.0)); // Increase amplitude
                ...
                // Create a gain node to boost microphone signal
                //const gainNode = audioContext.createGain();
                //gainNode.gain.value = 1; // Increase to desired gain (1.0 = no change)
    ...
    audioContext.audioWorklet.addModule("/lib/voicecall/playback-processor.js")
        .then(() => {
